...
const Directory: React.FC = () => {
  ...
  const [directoryTab365, setDirectoryTab365] =
    useState<DirectoryTab365>("all");
  const [directoryTabVendor, setDirectoryTabVendor] =
    useState<DirectoryTabVendor>("open");
  ...
  const [vendorCompanyName, setVendorCompanyName] = useState<string | null>(
    null,
  );
  const [projectHasVendorSubmission, setProjectHasVendorSubmission] = useState<
    Record<string, boolean>
  >({});
  const [projectVendorBidAmountMap, setProjectVendorBidAmountMap] = useState<
    Record<string, string | null>
  >({});
  const [projectHasAnyVendorSubmission, setProjectHasAnyVendorSubmission] =
    useState<Record<string, boolean>>({});
  const [vendorSubmissionOpen, setVendorSubmissionOpen] = useState(false);
  const [vendorSubmissionProject, setVendorSubmissionProject] =
  ...
  const isProjectAssignedToCurrentVendor = useCallback(
    (project: Project) => {
      ...
      const assigned = (project.V_BidDescription ?? "")
        .toString()
        .trim()
        .toLowerCase();
      ...
      if (tab === "open") {
        list = list.filter(
          (p) =>
            ...
            !projectHasVendorSubmission[String(p.id)],
        );
  ...
  const loadExistingAttachments = useCallback(async (projectId: string) => {
    const token = await getAccessTokenByApp();
    if (!token) return [];
    try {
      const folderId =
  ...
  const loadVendorFilesByCategory = useCallback(async (projectId: string) => {
    const token = await getAccessTokenByApp();
    if (!token) return [];
    try {
  ...
  const handleSaveProject = async (
    ...
    try {
      ...
      if (!trimmedName) {
        setProjectFormError(
          "Please fill all required fields: Project name. Attachments are optional.",
        );
        return;
      }
        ...
        if (attachmentIdsToDelete?.length) {
          ...
          if (token) {
            ...
            for (const fileId of attachmentIdsToDelete) {
              try {
                await sharePointService.deleteFile(token, containerId, fileId);
                deleted++;
              } catch (err) {}
            }
            if (deleted > 0) {
      ...
      toast({
        title: editingProject?.id ? "Project updated" : "Project created",
        variant: "success",
      });
  ...
  const handleVendorSubmissionSave = async (
    ...
    try {
      ...
      if (!token) {
        toast({
          title: "Directory access token error",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!vendorFolderId) {
        toast({
          title: "Directory vendor folder missing",
          variant: "destructive",
        });
        return;
      }
    ...
    <div className={styles.page}>
      <nav className={styles.topNav}>
        <div className={styles.navLeft}>
          <span className={styles.logo}>Projects</span>
        </div>
        ...
        <div className={styles.mainScroll}>
              ...
              {isVendor ? (
                ...
                <>
                  <button
                    type="button"
                    className={
                      directoryTab365 === "all" ? styles.tabActive : styles.tab
                    }
                  ...
                  <button
                    type="button"
                    className={
                      directoryTab365 === "open" ? styles.tabActive : styles.tab
                    }
              ...
              <tbody>
                {projectsLoading
                  ? Array.from({ length: 6 }).map((_, i) => (
                      <tr key={`skeleton-${i}`}>
                        <td>
                          <Skeleton className={styles.skeletonCellName} />
                        </td>
                        <td>
                        ...
                        <td>
                          <Skeleton className={styles.skeletonCellPill} />
                        </td>
                        <td>
                            ...
                            {isVendor ? (
                              !projectHasVendorSubmission[
                                ...
                                <button
                                  ...
                                  className={styles.actionBtn}
                                  onClick={() =>
                                    handleOpenVendorSubmission(project)
                                  }
          ...
          {isVendor && vendorSubmissionProject && (
            <VendorSubmissionDialog
              ...
              vendorCompanyName={
                vendorCompanyName ?? vendorUser?.username ?? ""
              }
          ...
          {!isVendor && (
            <UiDialog
              open={newProjectModeOpen}
              onOpenChange={setNewProjectModeOpen}
            >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-500">
                              Manually configure all project details and
                              timelines.
                            </span>
                          </div>
                        </div>
                        <div className="text-[11px] font-medium text-slate-400">
                          Best when you already know the full scope and
                          requirements.
                        </div>
                      </button>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-500">
                              Describe your idea and let AI draft the project
                              for you.
                            </span>
                          </div>
                        </div>
                        <div className="text-[11px] font-medium text-[#4b3ac9]">
                          Ideal for quick setup, suggestions and discovering
                          missing details.
                        </div>
                      </button>
                    </div>
                ...
                <AlertDialogHeader className="gap-3 sm:flex-row sm:items-start sm:text-left space-y-0">
                  ...
                  <div className="space-y-1 min-w-0">
                    ...
                    <AlertDialogDescription className="text-sm text-zinc-500 leading-snug max-w-sm">
                      This will permanently remove the project from the list.
                      Project attachments and related data will no longer be
