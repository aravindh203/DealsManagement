...
async CreateItem(
  ...
  async CreateColumn(token: string, containerId: string): Promise<any> {
    try {
      debugger;
      //  const GetList = `https://graph.microsoft.com/beta/storage/fileStorage/containers/${containerId}/columns`;
      // const responseList = await fetch(GetList, {
      //       method: 'GET',
      //       headers: {
      //         'Authorization': `Bearer ${token}`,
      //         'Content-Type': 'application/json'
      //       }})
      //       console.log('Existing columns:', await responseList.json());
